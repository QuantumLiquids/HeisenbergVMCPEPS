...
See `skills/cluster-info.md` for full cluster reference (SSH connection,
hardware, compilers, build instructions, known issues).
...
- `vmc/energy/optimization_log.jsonl` -- per-iteration structured log
  (auto-generated by upstream PEPS since commit e572888). Fields include
  iter, lr, energy, error, grad_norm, accept_rate, sr_iters, eval_t,
  selector_t, update_t, etc. Preferred over CSV for analysis.
