...
#ifndef QLPEPS_ALGORITHM_VMC_UPDATE_MODEL_SOLVERS_SPIN_ONEHALF_WAVEFUNCTION_FLUTRUATION_HEISENBERG_SQRPEPS_H
#define QLPEPS_ALGORITHM_VMC_UPDATE_MODEL_SOLVERS_SPIN_ONEHALF_WAVEFUNCTION_FLUTRUATION_HEISENBERG_SQRPEPS_H
...
class SpinOneHalfWaveFunctionFluctuationSqrPEPS : public ModelEnergySolver<TenElemT, QNT>,
                                                  ModelMeasurementSolver<TenElemT, QNT> {
  using SITPS = SplitIndexTPS<TenElemT, QNT>;
 public:
  SpinOneHalfWaveFunctionFluctuationSqrPEPS(void) = default;
...
TenElemT SpinOneHalfWaveFunctionFluctuationSqrPEPS<TenElemT, QNT>::CalEnergyAndHoles(const SITPS *split_index_tps,
  ...
  if (estimate_wavefunction_bias > 0.03) {
    std::cout << "inconsistent wave function amplitudes : "
              << "(" << min_abs << ", " << max_abs << ")"
              << std::endl;
    return false;
  }
  return estimate_wavefunction_bias;
}

template<typename TenElemT, typename QNT>
template<typename WaveFunctionComponentType>
ObservablesLocal<TenElemT> SpinOneHalfWaveFunctionFluctuationSqrPEPS<TenElemT, QNT>::SampleMeasure(
...
template<typename TenElemT, typename QNT>
std::vector<TenElemT> SpinOneHalfWaveFunctionFluctuationSqrPEPS<TenElemT,
...
#endif //QLPEPS_ALGORITHM_VMC_UPDATE_MODEL_SOLVERS_SPIN_ONEHALF_WAVEFUNCTION_FLUTRUATION_HEISENBERG_SQRPEPS_H
