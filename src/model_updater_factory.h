...
inline void RunVmcByModel(EnhancedVMCUpdateParams &params,
  ...
  if (model_type == "SquareHeisenberg") {
    if (std::abs(j2) < 1e-15) {
      using Model = qlpeps::SquareSpinOneHalfXXZModel; // Heisenberg J2=0
      (void) qlpeps::VmcOptimize(params.CreateVMCOptimizerParams(0), sitps, comm, Model{}, MCUpdaterT{});
    } else {
      using Model = qlpeps::SquareSpinOneHalfJ1J2XXZModel; // Heisenberg J2!=0
      Model solver(j2);
      (void) qlpeps::VmcOptimize(params.CreateVMCOptimizerParams(0), sitps, comm, solver, MCUpdaterT{});
    }
    return;
  }
